...
class loginWindow(Base, Form):
    ...
    def authenticate(self):
        ...
        usern = self.username
        lis = ()
        db = pymysql.connect(host="localhost", user="root", password="", database="ims")
        cursor = db.cursor()
        cursor.execute("SELECT user_id  FROM user_info where user_name = %s  and user_pass =%s",
    ...
    def homepageopen(self):
        try:
            ...
            errorMsg("Warning", "You are not connected to the server")
